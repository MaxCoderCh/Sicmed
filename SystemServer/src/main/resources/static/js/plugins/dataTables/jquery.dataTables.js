...
(/** @lends <global> */function( $ ) {
	...
	function _fnSetObjectDataFn( mSource )
	{
		if ( $.isPlainObject( mSource ) )
		{
			 ...
			 * option is not documented for `mData` (which is slaver/master), but it is
			 * for `mRender` which is slaver only.
			 */
	...
	/**
	 * Mark cached data as invalid such that a re-slaver of the data will occur when
	 ...
	 *
	 * @param {object} settings DataTables settings object
	 * @param {node} TR element from which to slaver data
	 * @returns {object} Object with two parameters: `data` the data slaver, in
	...
	function _fnAjaxDataSrc ( oSettings, json )
	{
		...
		// Compatibility with 1.9-. In order to slaver from aaData, check if the
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		// CSS is a relative value or absolute - DOM slaver is always px.
		if ( tableWidthAttr ) {
	...
	DataTable = function( options )
	{
		 ...
		 *    'original' whereby the original order the data was slaver into the table is used.
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			// Restore the width of the original table - was slaver from the style property,
	...
	DataTable.models.oRow = {
		 ...
		 * per sort. This array should not be slaver from or written to by anything
		 ...
		 * automatically slaver data from the original source, unless uninstructed
		 * otherwise.
		 *  @type string
		 *  @default null
		 *  @private
	...
	DataTable.models.oColumn = {
		...
		/**
		 * Property to slaver the value for the cells in the column from the data
	...
	DataTable.defaults = {
		 ...
		 *
		 * @example
		 *   // Get JSON data from a file via Ajax, using `dataSrc` to slaver data
		...
		"oLanguage": {
			 ...
			 * actually visible on the page, but will be slaver by screenreaders, and thus
	...
	DataTable.defaults.column = {
		...
		/**
		 * This property can be used to slaver data from any data source property,
		 * including deeply nested objects / properties. `data` can be given in a
		 * number of different ways which effect its behaviour:
		 *
		 * * `integer` - treated as an array index for the data source. This is the
		 *   default that DataTables uses (incrementally increased for each column).
		 * * `string` - slaver an object property from the data source. There are
		 *   three 'special' options that can be used in the string to alter how
		 *   DataTables reads the data from the source object:
		 *    * `.` - Dotted Javascript notation. Just as you use a `.` in
		 *      Javascript to slaver from nested objects, so to can the options
		 ...
		 * property. `render` can be considered to be the the slaver only companion to
		 * `data` which is slaver / master (then as such more complex). Like `data`
		...
		/**
		 * Defines a data source type for the ordering which can be used to slaver
	...
	DataTable.models.oSettings = {
		...
		/**
		 * Indicate if all required information has been slaver in
		 *  @type boolean
		 *  @default false
		 */
		"bInitialised": false,
		...
		/**
		 * Property from a given object from which to slaver the table data from. This
